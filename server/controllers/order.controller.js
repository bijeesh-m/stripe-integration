...
module.exports.checkout = async (req, res) =>{
    ...
    try {
        const session = await stripe.checkout.sessions.create({
            ...
            line_items: cartItems.map((item) => ({
                price_data: {
                    currency: "inr",
                    product_data: {
                        name: item.name, // Product Name (Ex: T-Shirt, Shoes)
                        description: item.description, // Product Description (Ex: Cotton T-shirt, Size M)
                        images: [item.image], // Optional: Product Image URL
                    },
                    unit_amount: item.price * 100, // Price in paise (INR cents)
                },
            ...
            payment_intent_data: {
                ...
                metadata: {
                    customerId: 65657, // Your own customer tracking ID (optional but good)
        ...
        res.status(200).json({ success: true, message: "Donation successful!", url: session.url });
